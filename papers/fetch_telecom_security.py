...
def fetch_telecom_updates():
    # 1. Fetch from Arxiv (Research)
    # Query: SS7 OR BGP OR SIP OR 5G Security
    arxiv_query = '(all:"SS7 security" OR all:"BGP hijacking" OR all:"SIP vulnerabilities" OR all:"5G security" OR all:"protocol security")'
    ...
    news_items = []
    keywords = ["ss7", "bgp", "sip", "voip", "5g", "lte", "telecom", "wireless", "protocol"]
    ...
    target_file = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'TELECOM_SECURITY_PROTOCOLS.md')
    ...
    if not papers:
        new_content += "No new papers found this week.\n"
    else:
        for p in papers:
    ...
    new_content += "\n### 📰 Latest News\n"
    if not news_items:
        new_content += "No telecom-specific news found this week.\n"
    else:
        for n in news_items:
